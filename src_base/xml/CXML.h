...
public:
	...
	const CStringPtr & operator = (const CStringPtr &ptr)
	{
		...
		{
			// delete m_ptr;
			// m_ptr 内存空间是由 strdup 中的malloc分配，释放时用 free 比较规范
			free(m_ptr);
		}
		if (ptr.m_ptr!=NULL)
		{
...
public:
	...
	///@param	name	名称
	///@param	value	值
	CXMLProperty(char *name=NULL, char *value=NULL);
...
//	virtual void output(CLogger *pLogger,int indent=0);
	...
	///@param	value	值
	///@return	1表示成功，0表示失败
	int addProperty(char *name, char *value);
	
	///增加一个属性
	///@param	name	名称
	///@param	value	值
	///@return	1表示成功，0表示失败
	int addProperty(char *name, int value);
	...
	///@param	name	名称
	///@return	1表示成功，0表示失败
	int removeProperty(char *name);
	
	///替换一个指定名称的属性的值，如果有多个属性都是这个名称，则替换第一个
	///@param	name	名称
	///@param	newValue	新的值
	///@return	1表示成功，0表示失败
	int replaceProperty(char *name, char *newValue);
	
	///根据名称，寻找一个属性，如果有多个属性都是这个名称，则只找第一个
	///@param	name	名称
	///@return	找到的属性
	CXMLProperty *findProperty(char *name);
...
//	virtual void output(CLogger *pLogger,int indent=0);
	...
	CXMLNode *findNode(XMLNodeType type, char *tagName, char *propertyName=NULL, char *propertyValue=NULL);
...
typedef int (* TXMLPropertyActionFunc)(char *value, void *parameter);
...
public:
	...
	///@return	0表示需要结束操作，>0表示继续处理
	virtual int stringHandler(char *string);
};
...
//	virtual void output(CLogger *pLogger,int indent=0);
	...
	///@return	1表示符合,0表示不符合
	int validNode(XMLNodeType type, char *tagName, char *propertyName=NULL, char *propertyValue=NULL);
	...
	int groupAction(TXMLActionFunc pFunc, void *parameter=NULL,
		XMLNodeType type=XMLTag, char *tagName=NULL, 
		char *propertyName=NULL, char *propertyValue=NULL);
	...
	///@param	propertyValue	要寻找的标记必须包含的属性值，NULL表示不需要检查属性值，当propertyName为NULL时，此参数无意义
	///@return	0表示操作被操作函数中断，其他值表示最后一次叫用操作函数时的返回值，-1表示没有可操作的对象
	int groupAction(char *usePropertyName, TXMLPropertyActionFunc pFunc, void *parameter=NULL,
		XMLNodeType type=XMLTag, char *tagName=NULL, 
		char *propertyName=NULL, char *propertyValue=NULL);
	...
	int XAction(char *EXPath, CXMLAction *pAction);
...
private:
	...
	int stringRefAction(char *pPath,CXMLAction *pAction);
	...
	///@return	返回结果按照XAction的定义
	int nodeCheck(char *pPath,CXMLAction *pAction, int depth);
	...
	int matchNode(char *pPath, char **pathTail, int *pIsMatch);
	...
	int getStringValue(char *pPath, char **pathTail, char **pStringValue);
	...
	char *getToken(char *pPath, char **pathTail);
};
...
///@param	propertyValue	要寻找的标记必须包含的属性值，NULL表示不需要检查属性值，当propertyName为NULL时，此参数无意义
///@return	0表示操作被操作函数中断，其他值表示最后一次叫用操作函数时的返回值，-1表示没有可操作的对象
int XMLGroupAction(CXMLNode *pNode, char *usePropertyName, TXMLPropertyActionFunc pFunc, void *parameter=NULL,
	XMLNodeType type=XMLTag, char *tagName=NULL, 
	char *propertyName=NULL, char *propertyValue=NULL);
...
///@param	propertyValue	要寻找的标记必须包含的属性值，NULL表示不需要检查属性值，当propertyName为NULL时，此参数无意义
///@return	0表示操作被操作函数中断，其他值表示最后一次叫用操作函数时的返回值，-1表示没有可操作的对象
int XMLGroupAction(CXMLNodeList *pNodeList, char *usePropertyName, TXMLPropertyActionFunc pFunc, void *parameter=NULL,
	XMLNodeType type=XMLTag, char *tagName=NULL, 
	char *propertyName=NULL, char *propertyValue=NULL);
...
//	virtual void output(CLogger *pLogger,int indent=0);
	...
	///@return	DTD文件名
	char *getDoctype(void);
	
	///设置DTD文件
	///@param	doctype	DTD文件
	void setDoctype(char *doctype);
	...
	void read(char *filename,int ignoreText=0, bool ignoreComment=true);
	...
	///@param	ignoreText	1表示忽略所有的正文，0表示不忽略正文
	///@exception	如果发生语法错误，则抛出CParseError
	void readString(char *string, int ignoreText=0);
...
private:
	...
	char *getNextName(FILE *input,int allowValue=0);
	...
	///@return	得到的字符串
	///@exception	发生语法错误，将抛出CParseError
	char *getString(FILE *input,char *endString);
...
public:
	...
	///@param	lineNo	错误的行号
	CParseError(char *msg, int lineNo);
