...
void TIniFile::ReadStringList(const char *section, const char *entry, const char *defaultString, vector<string> *temp)
{
	int len = -1;
	int bufLen = 0;
	char buffer[4096];
	...
	{
		char strmarket[4096];
		memset(strmarket, 0x0, sizeof(strmarket));

		memcpy(strmarket, buffer + 1, strlen(buffer) - 2);
		string str(strmarket);
		...
		{
			temp->push_back(str.substr(last, index - last));
			last = index + 1;
