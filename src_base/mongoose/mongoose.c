...
void mg_if_can_send_cb(struct mg_connection *nc) {
  ...
  if (n < 0) {
    nc->flags |= MG_F_CLOSE_IMMEDIATELY;
	//printf("[%12x]MG_F_CLOSE_IMMEDIATELY \n", nc);
	//将发送的数目变成0，不至于让上面的发送程序变成死循环
	nc->send_mbuf.len = 0;
  } else if (n > 0) {
    nc->last_io_time = (time_t) mg_time();
...
static void mg_ws_close(struct mg_connection *nc, const void *data,
  ...
  //xuzhonghua 这里如果发送了断开请求，多线程必须处理掉其他线程的请求
  mg_call(nc, nc->handler, nc->user_data, MG_EV_CLOSE, NULL);
    ...
    if (reass) {
      ...
      switch (flags & FLAGS_MASK_OP) {
        ...
        //  mg_send_websocket_frame(nc, WEBSOCKET_OP_PONG, wsm.data, wsm.size);
			//xuzhonghua,这里可以考虑回调函数，让上层的线程分发到发送线程去处理
		  mg_call(nc, nc->handler, nc->user_data, WEBSOCKET_OP_PING, &wsm);
          break;
